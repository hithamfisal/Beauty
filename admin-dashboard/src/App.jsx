...
const emptyAvailability = { beautician_id:'', availability_status:'available', working_days:['0','1','2','3','4','5'], work_start_time:'10:00', work_end_time:'22:00', max_daily_bookings:3, coverage_city_ids:[], coverage_district_ids:[] };
const availabilityLabels = { available:'متاحة', busy:'مشغولة', inactive:'موقوفة' };
...
function App() {
  ...
  const [smartSuggestions, setSmartSuggestions] = useState({ booking_id:null, items:[] });
  ...
  function chooseAvailabilityBeautician(id) {
    ...
    const parseArr = v => Array.isArray(v) ? v.map(String) : (()=>{ try { return JSON.parse(v||'[]').map(String); } catch { return []; } })();
    setAvailabilityForm({
      beautician_id: a.id,
      availability_status: a.availability_status || 'available',
      working_days: parseArr(a.working_days).length ? parseArr(a.working_days) : ['0','1','2','3','4','5'],
  ...
  async function saveAvailability(e) {
    e.preventDefault();
    if (!availabilityForm.beautician_id) return setMessage('اختر خبيرة التجميل أولاً.');
    try {
      await api(`/admin/beauticians/${availabilityForm.beautician_id}/availability`, { method:'PATCH', body: JSON.stringify(availabilityForm) });
      setMessage('تم حفظ توفر وتغطية خبيرة التجميل.');
      await load();
    } catch(e) { setMessage(`تعذر حفظ التوفر: ${e.message}`); }
  }

  async function suggestBeauticians(b) {
    try {
      const data = await api(`/admin/bookings/${b.id}/smart-beauticians`);
      setSmartSuggestions({ booking_id:b.id, booking_number:b.booking_number, items:data.suggestions || [] });
      const names = (data.suggestions || []).filter(x=>x.is_suitable).slice(0,5).map(x=>x.name).join('، ');
      setMessage(names ? `الخبيرات المرشحات للطلب ${b.booking_number || ''}: ${names}` : 'لا توجد خبيرات مناسبة حسب التوفر والتغطية.');
    } catch(e) { setMessage(`تعذر تحميل الترشيح الذكي: ${e.message}`); }
  }
  ...
  return <main dir="rtl">
    ...
    <section className="panel"><h2>إدارة خبيرات التجميل</h2><form className="grid4" onSubmit={saveBeautician}><Field label="اسم خبيرة التجميل"><TextInput required value={beauticianForm.name} onChange={v=>setBeautician('name',v)} /></Field><Field label="الجوال"><TextInput required value={beauticianForm.phone} onChange={v=>setBeautician('phone',v)} /></Field><Field label="المنطقة"><Select value={beauticianForm.region_id} onChange={v=>setBeautician('region_id',v)}><OptionList items={catalog.regions}/></Select></Field><Field label="المدينة"><Select value={beauticianForm.city_id} onChange={v=>setBeautician('city_id',v)}><OptionList items={citiesForBeautician}/></Select></Field><Field label="الخبرة الأساسية"><Select value={beauticianForm.main_expertise_service_id} onChange={v=>setBeautician('main_expertise_service_id',v)}><OptionList items={catalog.services} label="display_name" /></Select></Field><Field label="الأحياء التي تغطيها"><TextInput value={beauticianForm.districts} onChange={v=>setBeautician('districts',v)} /></Field><Field label="المهارات"><TextInput value={beauticianForm.skills} onChange={v=>setBeautician('skills',v)} /></Field><Field label="التقييم"><TextInput type="number" value={beauticianForm.rating} onChange={v=>setBeautician('rating',Number(v)||5)} /></Field><Field label="الحالة"><Select value={beauticianForm.status} onChange={v=>setBeautician('status',v)}><option value="active">فعالة</option><option value="inactive">غير فعالة</option></Select></Field><Field label="نبذة"><textarea value={beauticianForm.bio} onChange={e=>setBeautician('bio',e.target.value)} /></Field><div className="actions"><button>{editing.type==='beautician' ? 'حفظ التعديل' : 'إضافة خبيرة'}</button></div></form>
      <table><thead><tr><th>الاسم</th><th>الجوال</th><th>المدينة</th><th>الخبرة</th><th>التوفر</th><th>التقييم</th><th>إجراء</th></tr></thead><tbody>{beauticians.map(a=><tr key={a.id}><td>{a.name}</td><td>{a.phone}</td><td>{a.city_name||'-'}</td><td>{a.main_expertise_name||'-'}</td><td>{availabilityLabels[a.availability_status||'available']||a.availability_status||'متاحة'}</td><td>{a.review_rating||a.rating||'-'}</td><td><button onClick={()=>{setBeauticianForm({...emptyBeautician,...a});setEditing({type:'beautician',id:a.id});}}>تعديل</button><button onClick={()=>chooseAvailabilityBeautician(a.id)}>التوفر</button><button className="danger" onClick={()=>deleteItem('/admin/beauticians',a.id,'خبيرة التجميل')}>حذف</button></td></tr>)}</tbody></table></section>

    <section className="panel"><h2>توفر وتغطية خبيرات التجميل</h2><form className="grid4" onSubmit={saveAvailability}><Field label="خبيرة التجميل"><Select required value={availabilityForm.beautician_id} onChange={chooseAvailabilityBeautician}><OptionList items={beauticians} label="name" empty="اختر خبيرة" /></Select></Field><Field label="حالة التوفر"><Select value={availabilityForm.availability_status} onChange={v=>setAvailabilityForm({...availabilityForm,availability_status:v})}><option value="available">متاحة</option><option value="busy">مشغولة</option><option value="inactive">موقوفة</option></Select></Field><Field label="بداية العمل"><TextInput type="time" value={availabilityForm.work_start_time} onChange={v=>setAvailabilityForm({...availabilityForm,work_start_time:v})}/></Field><Field label="نهاية العمل"><TextInput type="time" value={availabilityForm.work_end_time} onChange={v=>setAvailabilityForm({...availabilityForm,work_end_time:v})}/></Field><Field label="الحد اليومي للطلبات"><TextInput type="number" value={availabilityForm.max_daily_bookings} onChange={v=>setAvailabilityForm({...availabilityForm,max_daily_bookings:Number(v)||1})}/></Field><Field label="أيام العمل"><select multiple value={availabilityForm.working_days} onChange={e=>setAvailabilityForm({...availabilityForm,working_days:Array.from(e.target.selectedOptions).map(o=>o.value)})}>{dayLabels.map((d,i)=><option key={i} value={String(i)}>{d}</option>)}</select></Field><Field label="المدن المغطاة"><select multiple value={availabilityForm.coverage_city_ids} onChange={e=>setAvailabilityForm({...availabilityForm,coverage_city_ids:Array.from(e.target.selectedOptions).map(o=>o.value)})}>{catalog.cities.map(c=><option key={c.id} value={c.id}>{c.name_ar}</option>)}</select></Field><Field label="الأحياء المغطاة"><select multiple value={availabilityForm.coverage_district_ids} onChange={e=>setAvailabilityForm({...availabilityForm,coverage_district_ids:Array.from(e.target.selectedOptions).map(o=>o.value)})}>{catalog.districts.map(d=><option key={d.id} value={d.id}>{d.name_ar}</option>)}</select></Field><div className="actions"><button>حفظ التوفر والتغطية</button></div></form><p className="muted">اترك المدن أو الأحياء بدون اختيار لتكون التغطية عامة لكل المدن/الأحياء.</p></section>

    {smartSuggestions.items.length>0 && <section className="panel"><h2>الترشيح الذكي للطلب {smartSuggestions.booking_number||''}</h2><div className="portfolio-grid">{smartSuggestions.items.slice(0,8).map(s=><div className="portfolio-card" key={s.id}><b>{s.name}</b><small>{s.main_expertise_name||'-'} • {s.city_name||'تغطية عامة'}</small><p>النتيجة: {Math.round(s.suitability_score||0)} — {s.is_suitable?'مناسبة':'غير مناسبة'}</p><p>{(s.suitability_reasons||[]).join('، ')}</p>{(s.suitability_warnings||[]).length>0&&<p className="danger-text">{s.suitability_warnings.join('، ')}</p>}<button onClick={()=>assignBeautician(smartSuggestions.booking_id,s.id)}>تعيين هذه الخبيرة</button></div>)}</div></section>}
